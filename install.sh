...
# -*- coding: utf-8 -*-
"""
Antigravity Quota HUD Daemon v4
...
Geniş ekranda tek satır; dar ekranda zarifçe iki satıra geçer.
"""
...
(() => {
    ...
    function ensureBar() {
        ...
        if (bar && (bar.getAttribute('data-v') !== '4' || bar.nextElementSibling !== inputCard)) {
            bar.remove();
            bar = null;
        }
        ...
        if (!bar) {
            ...
            bar.innerHTML = `
                ...
                <div id="dock-chunk-secondary" style="display:inline-flex; align-items:center; gap:6px; flex-shrink:0; white-space:nowrap; line-height:1;">
                    ...
                    <span style="color:#71717a; line-height:1;">Claude:</span>
                    <span id="dock-val-claude" style="font-family:monospace; color:#34d399; font-weight:600; line-height:1;">--%</span>
    ...
    if (!window.__agy_quota_v4_setup) {
        window.__agy_quota_v4_setup = true;
...
async def run_daemon():
    print("Starting Antigravity Responsive Quota HUD Daemon v4...", flush=True)
    while True:
        try:
            port = get_cdp_port()
            csrf = get_csrf()
                    ...
                    if ws_url:
                        try:
                            async with websockets.connect(ws_url, open_timeout=1.5, close_timeout=1.0) as ws:
                                check_expr = '!!document.getElementById("chat-dock-quota-bar") && document.getElementById("chat-dock-quota-bar").getAttribute("data-v") === "4" && !document.getElementById("antigravity-quota-hud") && !document.getElementById("test-quota-banner")'
                                is_clean = await cdp_eval(ws, check_expr)
                                if not is_clean:
                                    await cdp_eval(ws, code_to_inject)
                                    print(f"Updated Quota Bar v4 on: {p.get('title')}", flush=True)
                        except Exception:
                            pass
        except Exception:
            pass
